...
export async function POST(req: Request) {
  try {
    ...
    if (!file || !threadId) {
      return NextResponse.json(
        { success: false, message: "ファイルとスレッドIDが必要です。" }, 
        { status: 400 }
      );
    }
    ...
    if (file.size > MAX_FILE_SIZE) {
      return NextResponse.json(
        { success: false, message: "ファイルサイズが大きすぎます（5MB以下）。" }, 
        { status: 400 }
      );
    }
    ...
    if (!validateFileExtension(file.name)) {
      return NextResponse.json(
        { success: false, message: "対応していないファイル形式です。" }, 
        { status: 400 }
      );
    }
    ...
    const mainFileName = `post/${year}/${month}/${day}/${baseName}_${timeStamp}_0_size_${size}.${ext}`;
    const thumbFileName = `post/${year}/${month}/${day}/thumb_${baseName}_${timeStamp}_0_size_${size}.${ext}`;
    ...
    await Promise.all([
      uploadToPresignedUrl(buffer, presignedData[0].url, file.type),
      uploadToPresignedUrl(thumbnailBuffer, presignedData[1].url, file.type)
    ]);
    ...
    return NextResponse.json({ 
      ...
      message: "画像がアップロードされました（メイン画像とサムネイル画像）" 
    });
